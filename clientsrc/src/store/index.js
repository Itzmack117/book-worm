...
//https://www.googleapis.com/books/v1/volumes?maxResults=40&q=harrypotter
const googleApi = Axios.create({
  baseURL: "https://www.googleapis.com/books/v1/volumes?maxResults=40&q=",
  timeout: 3000,
});
  ...
  mutations: {
    ...
    searchBooks(state, books) {
      state.searchResults = books
        .filter((r) => r.volumeInfo.industryIdentifiers)
        .map((r) => {
          try {
            if (r.volumeInfo.industryIdentifiers[0].identifier) {
              return {
                ...
                authors: r.volumeInfo
                  ? r.volumeInfo.authors
                    ? r.volumeInfo.authors.toString()
                    : " "
                  : " ",
                ISBN: r.volumeInfo
                  ? r.volumeInfo.industryIdentifiers[0]
                ...
                description: r.volumeInfo
                  ? r.volumeInfo.description
                  : "No Description",
                price: r.saleInfo
                  ? r.saleInfo.listPrice
